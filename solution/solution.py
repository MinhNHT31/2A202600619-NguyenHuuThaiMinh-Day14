...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            relevance_flags.append(1 if overlap_ratio >= relevance_threshold else 0)
...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(item_averages) >= 2:
            ...
            others_avg = sum(item_averages[1:]) / len(item_averages[1:])
            if first_avg > others_avg:
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for qa in qa_pairs:
            ...
            res = evaluator.run_full_eval(answer, qa.question, qa.context or "", qa.expected_answer)
            res.qa_pair = qa
            if qa.retrieved_contexts:
                res.context_recall = evaluator.evaluate_context_recall(qa.retrieved_contexts, qa.expected_answer)
                res.context_precision = evaluator.evaluate_context_precision(qa.retrieved_contexts, qa.expected_answer)
            results.append(res)
        return results
    ...
    def run_regression(self, new_results: list[EvalResult], baseline_results: list[EvalResult]) -> dict[str, Any]:
        ...
        def get_avgs(results):
            ...
            r = sum(r.relevance for r in results) / total
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list[EvalResult], suggestions: list[str]) -> str:
        ...
        for i, f in enumerate(failures):
            ...
            sugg = suggestions[i] if i < len(suggestions) else ""
    ...
    def generate_improvement_suggestions(
        ...
        sorted_types = sorted(categories.items(), key=lambda x: x[1], reverse=True)
        for ftype, count in sorted_types:
            ftype_lower = ftype.lower()
            if ftype_lower == "hallucination":
                suggestions.append("Implement hallucination checker to filter unsupported claims")
            elif ftype_lower == "irrelevant":
                suggestions.append("Improve prompt clarity to prevent irrelevant responses")
            elif ftype_lower == "incomplete":
            ...
            elif ftype_lower == "off_topic":
                suggestions.append("Refine routing or classification step to handle out-of-domain queries")
